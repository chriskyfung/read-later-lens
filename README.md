...
| `source_file_id / name` | set from the imported file — or, when re-importing one of this app's own unified exports, restored from the source that originally owned the record                                |
...
- **A multi-file selection is refused up front rather than half-imported**: when a selected file collides with an existing source and the duplicate-choice dialog cannot be shown or answered, nothing is imported and the toast names the colliding file — `無法顯示同名檔案的處理選項，因此未匯入任何檔案（<file>）。請重新載入頁面後再試。`. A selection with no collisions imports normally regardless of that dialog's markup.
- **An unexpected mid-batch failure rewinds the whole batch**: every file that batch already imported is removed from the session _and_ from the cache (`匯入「<file>」時發生錯誤，已還原本次匯入的全部檔案`), so a multi-file import lands all-or-nothing instead of silently losing the files queued behind the failure. Failures reported for a single file — a parse error, an empty overwrite, a failed cache write — still affect only that file, and the rest of the selection continues.
- **Unified JSON exports are versioned**: `all_bookmarks_export.json` wraps rows in `{ "format": "read-later-lens", "version": 2, "sources": [...], "bookmarks": [...] }`. The envelope is recognized exactly on re-import; plain arrays and older exports keep working.
- **A unified export round-trips your folder structure.** The `sources` manifest records which source each group of bookmarks came from, and re-importing under the _Read Later Lens 統一匯出_ profile rebuilds one folder per source instead of collapsing the whole library into a single folder. Sources owning no exported record are left out, so a restore never conjures an empty folder. An older export with no manifest still restores correctly — its records carry the same source ids. A source id that is already loaded is merged into rather than replaced, so re-importing an export taken from the current session is safe, and the merged records keep that folder's own name rather than relabelling themselves from the manifest. Choosing 覆寫 for a name collision removes the old source first and then rebuilds whatever the file names, so an export that the user decides to overwrite still ends with a folder for every record. The unified CSV does the same from its per-row `source_file_id` / `source_file_name` columns.
- **Grouping is a unified-format behavior.** Under the _InstapaperScraper 匯出_ profile those ids are treated as the app's own, and the file imports as a single source.
- **A per-source save re-emits the source's own schema.** Saving a source back writes the columns that file actually had, in its own order (recorded at import as the CSV header row), rather than the app's internal unified fields — so re-importing your own file no longer warns that it "looks more like a Read Later Lens export". Unified-profile sources keep the full internal schema, and so do sources with no recorded profile, since guessing a narrower layout could silently drop fields. A column the app holds no value for is written empty and reported in the export toast. Column names are treated as untrusted keys throughout: a column called `constructor` or `__proto__` is written empty and counted, never read off `Object.prototype`.
- **A restored folder falls back to its profile's columns.** A folder rebuilt from a unified export has no file of its own behind it, and the manifest records which sources an export came from — never what shape they had. Rather than hand the restored folder the envelope's own header row, save-back uses its profile's columns, so a round trip never silently rewrites a folder's layout into the unified schema. A restored folder takes its file type from its own name — a manifest's `type` is used only when it names a type the app can write — so saving a folder called `raindrop.json` writes JSON rather than whatever format the export happened to be. A folder whose original file was a `.db` cannot be rebuilt at all: its table layout is not part of the manifest, so save-back says so instead of emitting a schema that merely resembles the original.
