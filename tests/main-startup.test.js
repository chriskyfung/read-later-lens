...
describe('entry-point feature wiring', () => {
  it('persists and renders uploads through the registered input listener', async () => {
    ...
    els.fileInput.dispatch('change', { target: { files: [{
      name: 'test.json',
  ...
  it('exports a source through a nested click after reopening the save modal', async () => {
    ...
    expect(saveFileWithFallback).toHaveBeenCalledWith(
      JSON.stringify(state.bookmarks, null, 2), 'test.json', 'application/json',
    );
  ...
  it.each([
    ...
    Object.values(els).find((el) => el.dataset.tab === tab).dispatch('click');
  ...
  function checkSources(dir) {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) checkSources(url);
        ...
        expect(readFileSync(url, 'utf8'), url.pathname).not.toMatch(/window\.IBM|onclick\s*=\s*["']/);
      }
    }
  }
...
function makeEl() {
  ...
  return {
    innerText: '', value: '', style: {}, dataset: {}, children: [],
    className: '', listeners: {}, _html: '',
    classList: {
    ...
    set innerHTML(html) { this._html = html; this.children = []; selectors.clear(); },
...
beforeEach(async () => {
  ...
  vi.stubGlobal('document', {
    ...
    querySelectorAll: (selector) => Object.entries(els)
      .filter(([id, el]) => selector === '.main-tab' ? Boolean(el.dataset.tab) : /^panel[A-Z]/.test(id))
      .map(([, el]) => el),
  });
...
const bookmark = (id, title, url) => ({
  id, title, url, tags: [], detected_language: 'en', source_file_name: 'test.json',
});
...
describe('module-owned startup', () => {
  it('mounts, wires listeners and renders empty state once without publishing globals', async () => {
    ...
    for (const [id, type] of [
      ['fileInput', 'change'], ['searchInput', 'input'], ['saveBackBtn', 'click'],
      ['bookmarkCardsGrid', 'click'], ['folderList', 'click'], ['tagFilterCloud', 'click'],
