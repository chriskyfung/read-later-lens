...
- **Harden exported CSV cells against formula injection.** A cell starting with `=`, `+`, `-`, `@`, TAB or CR is evaluated as a formula (and historically a DDE command) when the user opens the export in a spreadsheet, so the same untrusted data that is escaped for markup is also dangerous on the way out. Route the rows through `hardenRecordsForCsv()` (`src/utils/csv.js`) immediately before `Papa.unparse` in `src/io/exporter.js` — both the per-source save and the unified CSV export. Harden a **copy**: the in-memory records, and therefore the JSON and SQLite exports, must stay byte-faithful. Judge arrays (`tags`) on the joined cell text, not per element, so a list like `news,=1+1` round-trips unchanged instead of picking up a spurious quote. JSON and SQLite exports deliberately stay untouched — spreadsheets are not a threat model for them.
